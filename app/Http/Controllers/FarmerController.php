...
class FarmerController extends Controller
{
    public function index()
    {
        ...
        if (Auth::user()->role_id == 2) {
            $data['farmer'] = Farmer::with(['farm','city','district','province','country'])->where('user_id', Auth::user()->id)->first();
    ...
    {
        $farmer = Farmer::with(['farm','city','district','province','country'])->findOrFail($id);
        return view('farmer.viewmore', ['farmer' => $farmer]);
    }

    public function myProfile()
    {
        $farmer = Farmer::with(['farm','city','district','province','country'])->where('user_id', Auth::user()->id)->firstOrFail();
        return view('farmer.viewmore', ['farmer' => $farmer]);
    }

    public function editProfile()
    {
        $farmer = Farmer::where('user_id', Auth::user()->id)->firstOrFail();
    ...
    public function updateProfile(Request $request)
    {
        ...
        // Update the user's basic info too if they changed their name/phone
    ...
    public function store(Request $request)
    {
        ...
        $user = User::create([
            ...
            'phone' => $request->phone,
            'email' => $request->email ?? $request->phone.'@soili.com',
            'role_id' => 2,
        ...
        if ($user) {
            ...
            $data = $request->all();
            $data['email'] = $request->email ?? $request->phone.'@soili.com';
